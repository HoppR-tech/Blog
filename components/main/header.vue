  ...
  <div>
    <div class="bg-hoppr-purple fixed top-0 left-0 right-0 z-50">
      <nav class="container px-6 py-2 mx-auto">
        ...
        <div class="hidden md:flex md:items-center md:justify-between mt-4">
          <ul class="flex space-x-4">
            <li v-for="(item, index) in menuItems" :key="index">
              <NuxtLink
                :to="item.to"
                class="text-gray-100 hover:text-hoppr-green"
    ...
    >
      <div v-if="showMenu" class="fixed top-0 left-0 right-0 bottom-0 bg-indigo-600 z-50 md:hidden">
        ...
        <ul class="flex flex-col mt-16 space-y-2 px-6 py-4">
          <li v-for="(item, index) in menuItems" :key="index">
            <NuxtLink
              :to="item.to"
              class="block py-2 text-gray-100 hover:text-hoppr-green"
              :class="{ 'text-hoppr-green font-semibold': path === item.path && item.path !== 'about' }"
              :target="item.path === 'about' ? '_blank' : '_self'"
              :rel="item.path === 'about' ? 'noopener noreferrer' : ''"
              @click="closeMenu"
