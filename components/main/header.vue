                  ...
                  <li v-for="(item, index) in menuItems" :key="index" class="relative">
                    <NuxtLink :to="item.to"
                      ...
                      <span
                        ...
                        :class="{ 'scale-x-100': (path === item.path && item.path !== 'about') || (item.path === 'blogs' && isSearchActive) }"></span>
                    </NuxtLink>
                  </li>
                </ul>
              </div>
            ...
            <div class="hidden lg:flex lg:items-center space-x-4">
              ...
              <ClientOnly>
                <button v-if="colorMode.value === 'light'"
                  ...
                  <LazyIcon name="mdi:weather-night" size="24"
                    class="transform transition-transform duration-300 group-hover:rotate-12" />
                </button>
                <button v-else
                  class="p-2 text-hoppr-green hover:text-hoppr-green rounded-full transition-colors duration-300 group"
                  aria-label="Activer le mode clair" @click="onClick">
                  <LazyIcon name="mdi:weather-sunny" size="24"
                    class="transform transition-transform duration-300 group-hover:rotate-12" />
                </button>
              </ClientOnly>
            </div>
            <div class="lg:hidden">
              <button class="text-gray-100 focus:outline-none" aria-label="Ouvrir le menu de navigation"
                @click="toggleNav">
      ...
      <div v-if="showMenu" class="fixed top-0 left-0 right-0 bottom-0 bg-hoppr-purple z-50 lg:hidden">
        ...
        <ul class="flex flex-col mt-16 space-y-2 px-6 py-4">
          <li v-for="(item, index) in menuItems" :key="index">
            <NuxtLink :to="item.to" class="block py-2 text-gray-100 hover:text-hoppr-green uppercase tracking-wider"
          ...
          <li class="flex items-center justify-between py-2 uppercase tracking-wider" @click="onClick">
            ...
            <ClientOnly>
              <button class="p-2 text-white hover:text-hoppr-green rounded-full transition-colors duration-300 group"
                :aria-label="colorMode.value === 'light' ? 'Activer le mode sombre' : 'Activer le mode clair'">
                <LazyIcon :name="colorMode.value === 'light' ? 'mdi:weather-night' : 'mdi:weather-sunny'" size="20"
                  class="transform transition-transform duration-300 group-hover:rotate-12" />
              </button>
            </ClientOnly>
          </li>
