...
![Olivier Breda, debout derrière un pupitre équipé d'un ordinateur portable, présente sa conférence à DevLille. Derrière lui, un grand écran affiche le titre de la présentation : "Failure MUST be an option — Failures happen. What matters is how your organization responds](./assets/img1.webp)
...
![Diapositive projetée sur la scène de DevLille, intitulée "Onboarding Journey — Steps by Steps" du DevHub d'Adéo. Elle présente le parcours d'intégration des développeurs en 8 étapes numérotées et reliées : création des comptes du nouvel employé, pré-onboarding créé selon le poste, email d'onboarding envoyé au parrain/manager, personnalisation par le parrain, email envoyé au nouvel arrivant, parcours d'onboarding personnalisé en autonomie, suivi pour le parrain et le nouvel arrivant, et collecte de feedback.](./assets/img2.webp)
...
![Diapositive projetée sur la scène de DevLille présentant les quatre dimensions d'expertise des rôles Staff Engineer et Principal Engineer, organisées en deux axes croisés. L'axe vertical va de Technology (define, evolve, evangelize, master, adopt) vers People (learn, help, mentor, organize, design). L'axe horizontal va de Visibility (team, guild, tech dir, company, industry) vers Business (app, domain, tribe, business line, company).](./assets/img3.webp)
